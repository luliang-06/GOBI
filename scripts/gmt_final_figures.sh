...
OUT_DIR="${BASE_DIR}/outputs/out_figs/"
...
dem_zi="${BASE_DIR}/data/dem_zi.nc"
dem_zo="${BASE_DIR}/data/dem_zo.nc"
dem_china="${BASE_DIR}/data/dem_china.nc"

f_033D="${BASE_DIR}/data/033D_05106_131313_boundary.gmt"
f_135D_023="${BASE_DIR}/data/135D_05023_131313_boundary.gmt"
f_135D_222="${BASE_DIR}/data/135D_05222_131313_boundary.gmt"
f_128A="${BASE_DIR}/data/128A_05172_131313_boundary.gmt"
f_055A_021="${BASE_DIR}/data/055A_05021_131313_boundary.gmt"
f_055A_221="${BASE_DIR}/data/055A_05221_131313_boundary.gmt"


# Figure 1: Study Region
echo "Starting Figure 1..."
gmt begin ${OUT_DIR}/F1_study_region png
	gmt subplot begin 1x2 -Fs10c/11.5c -M0.1c -A+gwhite+p0.5p,black+c0.1c
		# left panel: data
		gmt subplot set 0
			gmt basemap -R99.75/105.75/36.25/41.25 -JX10c/11.5c -B1 -BWeSn
			gmt grdimage $dem_zo -I${BASE_DIR}/data/shade_zo.nc -Cgray -t30
			gmt plot $basin -W0.8p,black
			gmt plot $f_033D        -W1.0p,50/100/200
			gmt plot $f_135D_023    -W1.0p,50/100/200
			gmt plot $f_135D_222    -W1.0p,50/100/200
			gmt plot $f_128A        -W1.0p,50/100/200
			gmt plot $f_055A_021    -W1.0p,50/100/200
			gmt plot $f_055A_221    -W1.0p,50/100/200
			awk -F "," '{print $2, $3}' $gps | gmt plot -St0.2 -G210/77/77 -W0.1p,black
			awk -F "," '{print $3, $4}' $points | gmt plot -Sc0.15 -G138/208/255 -W0.1p,black
			# inset figure of China map
			gmt inset begin -DjTL+w4.0c/2.9c+o0.64c/0c -F+p0.5p+gwhite
				gmt grdimage $dem_china -R70/140/15/55 -I${BASE_DIR}/data/shade_china.nc -JM4.0c -Crelief -t35
				gmt coast -ECN+p0.2p,black -ETW+p0.2p,black -ECN.GS+p0.1p,180/56/56
				gmt plot $basin -W0.1p,210/0/0 -G210/0/0@20
			gmt inset end
		# right panel: landcovers
		gmt subplot set 1
			gmt basemap -R101.2/104.0/37.0/39.5 -JX10c/11.5c -B1 -BwESn
			gmt grdimage $dem_zi -I${BASE_DIR}/data/shade_zi.nc -Cgray -t30
			gmt clip $basin
			gmt grdimage $landcover -Clandcover.cpt -T0/1 -Q -nn
			gmt clip -C
			gmt makecpt -Cacton -I
			gmt grdimage $permafrost -C -n+c -Q -t15
			gmt plot $river -W0.6p,#0a33ff
			gmt plot $glacier -W0.8p,#00f7ff
			gmt plot $basin -W0.8p,black
			cat <<- 'EOF' | gmt legend -DjTL+w3.0c+o0.64c/0c -F+p0.5p+g255/255/255@30 --FONT_ANNOT_PRIMARY=10p
				G 0.1c
