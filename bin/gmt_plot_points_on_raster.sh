...
gmt begin ${OUT_DIR}/time_lag_on_vu_a png,pdf
	...
	gmt plot $basin -W0.8p,black
	awk -F "," '(NR>1){lprint $3, $4, $30}' $points | gmt plot -Sc0.22 -Ccpts/timelag.cpt -W0.4p,black
    gmt colorbar -DjTL+w1.25i/0.12i+o1/1+h+e+ml -Cvu.cpt -F+gwhite+p0.1p -Bx10+l"Velocity (m/yr)" --FONT_ANNOT_PRIMARY=18p
	gmt colorbar -DjTR+w1.25i/0.12i+o1/1+h+e+ml -Ctimelag.cpt -F+gwhite+p0.1p -Bx160+l"Time lag (days)" --FONT_ANNOT_PRIMARY=18p
gmt end
...
gmt begin ${OUT_DIR}/time_lag_on_vu_d png,pdf
	...
	awk -F "," '(NR>1){lprint $3, $4, $30}' $points | gmt plot -Sc0.22 -Ccpts/timelag.cpt -W0.4p,black
    gmt colorbar -DjTL+w1.25i/0.12i+o1/1+h+e+ml -Cvu.cpt -F+gwhite+p0.1p -Bx10+l"Velocity (m/yr)" --FONT_ANNOT_PRIMARY=18p
	gmt colorbar -DjTR+w1.25i/0.12i+o1/1+h+e+ml -Ctimelag.cpt -F+gwhite+p0.1p -Bx60+l"Time lag (days)" --FONT_ANNOT_PRIMARY=18p
gmt end
